...
export namespace JWT {
     ...
     */
    export function sign<T extends object = any>(payload: T, options?: SignOptions): string {
        if (!payload || typeof payload !== 'object' || Array.isArray(payload)) throw new InternalServerError('Invalid payload');
        if (!options) options = {};
     ...
     */
    export function decode<T extends object = any>(token: string, options: DecodeOptions = {json: true}): T {
        if (!token || typeof token !== 'string') throw new InternalServerError('Invalid token');
