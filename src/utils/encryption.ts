...
export namespace Encryption {
    ...
    export function randomString(length: number, chars: string) {
        if (!chars) throw new InternalServerError("Argument 'chars' is undefined");

        const charsLength = chars.length;
        if (charsLength > 256)
            throw new InternalServerError("Argument 'chars' should not have more than 256 characters" + ', otherwise unpredictability will be broken');
    ...
    export function encrypt(data: string, pepper: string, iterations: number = 10000, salt?: string): string {
        if (!data) throw new InternalServerError('Data must be defined');
        if (typeof data !== 'string') throw new InternalServerError('Data must be a string');
